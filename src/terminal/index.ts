...
export default function Terminal(screenTextEngine: {
  ...
  window.addEventListener('keypress', e => {
    if (textarea.readOnly === true || document.activeElement?.id !== 'textarea') {
      ...
      if (e.key.length === 1) {
        textarea.value =
          textarea.value.slice(0, lastSelection) + e.key + textarea.value.slice(lastSelection)
        lastSelection += 1
        onInput()
      }
      textarea.setSelectionRange(lastSelection, lastSelection)
    ...
    if (lenDiff === 0) {
      ...
      while (op < oldStr.length || np < newStr.length) {
        ...
        if (oldStr.charAt(op) !== newStr.charAt(np)) {
          if (change.loc === 'none') change.loc = np === newStr.length ? 'end' : np
      ...
      while (op < oldStr.length || np < newStr.length) {
        ...
        if (oldStr.charAt(op) !== newStr.charAt(np)) {
          if (change.loc === 'none') change.loc = op === oldStr.length ? 'end' : op
