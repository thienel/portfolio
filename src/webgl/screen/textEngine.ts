...
export default function ScreenTextEngine(assists: Assists, sceneRTT: THREE.Scene) {
  ...
  function updateCharUnderCaret(isBlack: boolean) {
    if (charUnderCaret)
      (charUnderCaret.material as THREE.MeshBasicMaterial).color = new THREE.Color(
        isBlack ? 'black' : textColor,
      )
  }
  ...
  function mergeGeometriesWithMesh(baceMesh: THREE.Mesh, geometries: THREE.BufferGeometry[]) {
    ...
    baceMesh.geometry = mergeGeometries(geometries)
  ...
  function userInput(change: Change, selectionPos: number) {
    if (change.type === 'add') {
      ...
      if (change.loc === 'end') {
        ...
        inputBuffer = [
          ...inputBuffer.slice(0, change.loc),
          ...inputBuffer.slice(change.loc + change.str.length, inputBuffer.length),
        ]
        updateCharPos(
          inputBuffer,
          (obj, x, y) => {
